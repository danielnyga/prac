...
def main():
    ...
    parser.add_argument('--batch', '-b', nargs='+', dest='batch', default=None, help='Import a list of howtos in batch processing whose filenames are given the respective howto title, e.g. "Make a pancake." The file content must then be given by the single instruction steps, one by line.')
    parser.add_argument('--recursive', '-r', action='store_true', dest='recursive', default=False,  help='Apply the import of instructions recursively to subdirectories.')
    ...
    if args.verbose and args.howto:
        ...
        for path in args.batch:
            if args.recursive:
                ...
                for filename in os.listdir(path):
                    if filename == 'result.bson': continue
    ...
    if args.verbose:
        print('importing %s howtos:' % len(howtos))
        for howto in howtos:
            print(howto)
    ...
    try:
        ...
        pool.map(import_howto, list(zip(howtos, itertools.repeat(args.verbose), itertools.repeat(args.save))))
...
def import_howto(args):
    try:
        ...
        for howto, steps in howto_steps.items():
