...
class Grounding(PRACModule):
    ...
    def __call__(self, infer, worldmodel, constraints=None):
        if self.prac.verbose > 0:
            print(prac_heading('Grounding plan to worldmodel'))
        plangen = self.prac.module('plan_generation')
        return [plangen.makeplan(FrameNode(infer, f, None)) for f in self.deterministic_grounding(infer, worldmodel, constraints)]
    ...
    def deterministic_grounding(self, infer, worldmodel, constraints):
        ...
        for frame in frames:
            skipframe = False
            # out('grounding', frame)
            logger.debug('grounding', frame)
            for role, obj in frame.actionroles.items():
                if role in ('action_verb', 'unit', 'amount'):
                    continue
                ntypes = list(set([o.type for o in worldmodel.getall(obj)]))
                logger.debug(role, 'can be filled by', ntypes)
                out(role, 'can be filled by', ntypes)
                ...
                if not ntypes:
                    if not frame.mandatory:
                        ...
                        similars = worldmodel.getsimilars(obj, infer.similarity)
                        if similars:
